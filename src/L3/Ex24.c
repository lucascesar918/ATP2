...
void print_employees(const EmployeeList *el) {
  ...
  for (int i = 0; i < el->registered; i++) {
    ...
    printf("%s, Departamento de %s, Salário: %.2f\n", e.name,
           get_department_name(e.department), e.wage);
  }
}
...
void ui_add(EmployeeList *el) {
  if (el->registered >= el->capacity) {
    printf(
        "A equipe está cheia. Não é possível adicionar mais funcionários.\n");
    return;
  }
...
int main() {
  EmployeeList equipe = {"Sala 1", 0, EMPLOYEE_CAPACITY,
                         malloc(EMPLOYEE_CAPACITY * sizeof(Employee))};
  Employee e;
